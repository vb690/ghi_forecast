...
from modules.models.torch_backend.neural_networks.blocks import get_dense_block
...
class MLPModel(nn.Module):
    ...
    def __init__(
        ...
        self._weights = Linear(in_features=layers[-1], out_features=out_features)
        self._link_function = getattr(nn, link_function)()

    def forward(self, X):
        z = self._weights(self._dense_block(X))
        yhat = self._link_function(z)
        return yhat
